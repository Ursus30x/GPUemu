...
| `UEFI/OptionRom/gop3d.c` | `GpuCmdBindCompShader()`, `GpuCmdBindSSBO()`, `GpuCmdDispatchCompute()` |
...
The UEFI protocol exposes `GpuCmdBindCompShader`, `GpuCmdBindSSBO`, and
`GpuCmdDispatchCompute`. A typical sequence is:
...
GpuCmdBindCompShader(gop, shader_addr, shader_size);
GpuCmdBindSSBO(gop, 0, a_addr, input_size);
GpuCmdBindSSBO(gop, 1, out_addr, output_size);
GpuCmdDispatchCompute(gop, 1, 1, 1);
GpuPresent(gop);
GpuCmdReadBuffer(gop, out_addr, output, output_size);
```
