...
Application (UEFI / GOP3D)
  ...
  └─► Bind Texture: GpuCmdBindTexture(slot, hTexDesc) ──► Ring Buffer (CMD_SET_STATE, STATE_ID_TEXTURE_CONFIG)
                                                              │
                                                              ▼
                                                   GPU Emulator (QEMU)
                                                              │
...
typedef
EFI_STATUS
(EFIAPI *GOP_3D_CMD_BIND_TEXTURE)(
...
mGOP3D->GpuCmdBindTexture(mGOP3D, 1, hTexDesc); // Bind texture descriptor to slot 1
